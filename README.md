...
----
* General
    - Payload obfuscator; primarily for WARs 
    - Undeployers
    - Finish and test WebSphere
    - SOCKS proxy support (waiting on requests)
* JBoss
    - JMXInvokerServlet improvements
    - Several of the deployers that run serialized Java requests (bsh, webconsole invoker)
      don't currently support authentication.  Not sure what the bug is quite yet.
* Coldfusion
    - Deployers (all versions, multiple interfaces)
    - Other exploits, i.e. anything on exploit-db
    - Improve the RDP bypass, or turn it into a generic interface for executing functions in
      the coldfusion interface.
    - Fingerprints for older versions
    - Figure out how to implement the SMB hash retrieval method 
* Weblogic
    - Fingerprints for older versions
    - Need to test deployer against other versions
